...
from codecompasslib.chatbot.secrets_manager import load_openai_key, load_github_token, load_askthecode_api_base_url, \
...
def test_make_api_request_success(mock_github_token) -> None:
    ...
    with patch('codecompasslib.chatbot.api_utilities.requests.post') as mocked_post:
        mocked_post.return_value = Mock(status_code=200,
                                        json=lambda: {"data": "some data", "usefulUrls": ["http://example.com"]})
...
class TestRepoInfo:
    ...
    def test_get_repo_branches(self):
        ...
        with patch('codecompasslib.chatbot.repo_info.make_api_request', return_value=mock_response) as mocked_request:
            ...
            mocked_request.assert_called_once_with("https://mockapi.askthecode.ai/api/repository/branch/list",
                                                   {'url': "https://github.com/mockuser/mockrepo"})
            assert result == mock_response
